...
static void bstack(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    if (i < m->nmaster) {
      resize(c, mx, my,
             mw * (c->cfact / mfacts) + (i < (unsigned int)mrest ? 1 : 0) -
                 (2 * c->bw),
             mh - (2 * c->bw), 0);
      mx += WIDTH(c) + iv;
    } else {
      resize(c, sx, sy,
             sw * (c->cfact / sfacts) +
                 ((i - m->nmaster) < (unsigned int)srest ? 1 : 0) -
                 (2 * c->bw),
             sh - (2 * c->bw), 0);
      sx += WIDTH(c) + iv;
    }
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    if (!m->nmaster || i < m->nmaster) {
      resize(c, mx, my, mw - (2 * c->bw),
             mh * (c->cfact / mfacts) + (i < (unsigned int)mrest ? 1 : 0) -
                 (2 * c->bw),
             0);
      my += HEIGHT(c) + ih;
      ...
      resize(c, lx, ly, lw - (2 * c->bw),
             lh * (c->cfact / lfacts) +
                 ((i - 2 * m->nmaster) < (unsigned int)(2 * lrest) ? 1 : 0) -
                 (2 * c->bw),
             0);
      ly += HEIGHT(c) + ih;
    } else {
      resize(c, rx, ry, rw - (2 * c->bw),
             rh * (c->cfact / rfacts) +
                 ((i - 2 * m->nmaster) < (unsigned int)(2 * rrest) ? 1 : 0) -
                 (2 * c->bw),
             0);
      ry += HEIGHT(c) + ih;
    }
...
void gaplessgrid(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; i++, c = nexttiled(c->next)) {
    if ((int)(i / rows) + 1 > cols - (int)(n % cols)) {
      rows = n / cols + 1;
      ch = (m->wh - 2 * oh - ih * (rows - 1)) / rows;
      rrest = (m->wh - 2 * oh - ih * (rows - 1)) - ch * rows;
    }
...
void grid(Monitor *m) {
  ...
  for (rows = 0; rows <= (int)n / 2; rows++)
    if (rows * rows >= (int)n)
      break;
  cols = (rows && (rows - 1) * rows >= (int)n) ? rows - 1 : rows;
...
void horizgrid(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if ((int)i < ntop) {
      ...
      resize(c, sx, sy,
             sw * (c->cfact / sfacts) +
                 ((i - ntop) < (unsigned int)srest ? 1 : 0) - (2 * c->bw),
             sh - (2 * c->bw), 0);
      sx += WIDTH(c) + iv;
    }
}
